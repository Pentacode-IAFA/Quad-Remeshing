...
void MeshProcessingPlugin::registerPlugin( const Ra::Plugins::Context& context ) {
    ...
    connect(this, &MeshProcessingPlugin::askForUpdate, &context, &Ra::Plugins::Context::askForUpdate);
    RadiumMeshProcessing::RadiumMeshProcessingWelcome( "Initializing RadiumMeshProcessing library." );
...
bool MeshProcessingPlugin::doAddWidget( QString& name ) {
    ...
    connect( m_widget, &MyWidget::computationRequested, [this]( ProcessingParameter p ) {
        RadiumMeshProcessing::LaplacianSmoothing(p);
        emit askForUpdate();
    } );
    return true;
}
QWidget* MeshProcessingPlugin::getWidget()  {
    return m_widget;
}
