...
void RadiumMeshProcessingWelcome( const std::string& message ) {
    auto resourcesPath = Ra::Core::Resources::getResourcesPath(
        reinterpret_cast<void*>( &RadiumMeshProcessingMagick ), "Resources/MeshProcessingResources" );
    if ( !resourcesPath ) {
        LOG( logERROR )
            << RadiumMeshProcessingPrefix << " -- Unable to find resource directory. Abort";
        abort();
    }
    ...
    if ( inputMsg.is_open() ) {
        ...
        LOG( logINFO )
            << RadiumMeshProcessingPrefix << " -- " << message << buffer.str() << " at " << welcomeMsg;
    }
    else {
        LOG( Ra::Core::Utils::logINFO ) << RadiumMeshProcessingPrefix << " -- " << message << " -- "
...
void LaplacianSmoothing( ProcessingParameter p) {
    ...
    LOG( logINFO ) << RadiumMeshProcessingPrefix << " -- Laplacian smoothing on " << p.entityName << " // " << p.componentName;

    auto trianglemesh = dynamic_cast<Mesh*>( Ra::Engine::RadiumEngine::getInstance()->getMesh( p.entityName, p.componentName ) );
    auto polymesh = dynamic_cast<PolyMesh*>( Ra::Engine::RadiumEngine::getInstance()->getMesh( p.entityName, p.componentName ) );
    ...
    for ( auto v_it = topologicalMesh->vertices_begin(); v_it != topologicalMesh->vertices_end(); ++v_it ) {
    ...
    for ( e_it = topologicalMesh->edges_begin(); e_it != e_end; ++e_it )
    {
        ...
        d1.normalize();
        weight += 1.0_ra /
                  std::tan( acos( std::max( lb, std::min( ub, d0.dot( d1 ) ) ) ) );
        ...
        topologicalMesh->property( vertex_weights_, topologicalMesh->to_vertex_handle( heh0 ) ) += weight;
        topologicalMesh->property( vertex_weights_, topologicalMesh->to_vertex_handle( heh1 ) ) += weight;
    }
    ...
    for ( auto v_it = topologicalMesh->vertices_begin(); v_it != topologicalMesh->vertices_end(); ++v_it )
    {
        ...
        if ( weight != 0._ra )
            topologicalMesh->property( vertex_weights_, *v_it ) =
                1.0_ra / weight;
    }
    ...
    for ( auto v_it = topologicalMesh->vertices_begin(); v_it != topologicalMesh->vertices_end(); ++v_it )
    {
        ...
        for ( auto vv_it = topologicalMesh->vv_iter( *v_it ); vv_it.is_valid(); ++vv_it )
        {
    ...
    for ( auto v_it = topologicalMesh->vertices_begin(); v_it != topologicalMesh->vertices_end();
          ++v_it, ++cog_it )
    {
