...
QuadRemeshing_API void QuadRemesher::step1() {
    ...
    m_meshFilepath = "/home/mafo/Downloads/300/300/Mechanical/twirl20k.obj" ;
    //igl::writeOBJ(std::string("buc.obj"), m_mesh->getCoreGeometry().vertices().data()->derived(), m_mesh->getCoreGeometry().getIndices().data()->derived());
    ...
    loadConfigFile("/home/mafo/dev/Quad-RemeshingDemo/libs/quadwild/quadwild/basic_setup.txt", parameters);
    ...
    LOG(Ra::Core::Utils::logINFO)<<"Loaded "<<trimesh.fn<<" faces and "<<trimesh.vn<<" vertices";
...
QuadRemeshing_API void QuadRemesher::step3() {
  ...
  auto &attrib =
      m_mesh->getAttribArrayGeometry().getAttrib(m_mesh->getAttribArrayGeometry().getAttribHandle<Ra::Core::Vector4>("in_color"));
  ...
  int i = 0;
  std::ifstream stream("/home/mafo/Downloads/300/300/Mechanical/twirl20k_p0.patch");
  if(stream.is_open()){
